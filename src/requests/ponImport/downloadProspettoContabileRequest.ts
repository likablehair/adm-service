...
import { DownloadProspetto } from './downloadProspettoSintesiRequest';

export default class DownloadProspettoContabile extends BaseRequest<DownloadProspetto> {
  constructor() {
    const superArgs = {
  ...
  async processRequest(params: ProcessRequest<DownloadProspetto>): Promise<{
    ...
    try {
      ...
      return await this.asyncBaseProcessRequest({
        ...
        serviceId: 'downloadProspettoContabile',
      });
    } catch (error: unknown) {
